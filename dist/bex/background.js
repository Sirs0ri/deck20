(() => {
  ...
  var connections = {};
  var addConnection = (port2) => {
    const tab = port2.sender.tab;
    let connectionId;
    if (port2.name.indexOf(":") > -1) {
      const split = port2.name.split(":");
    ...
    currentConnection[port2.name] = {
      port: port2,
      connected: true,
      listening: false
    };
    return currentConnection[port2.name];
  };
  chrome.runtime.onConnect.addListener((port2) => {
    const thisConnection = addConnection(port2);
